...
TArray<FVector> getCirclePoints(int32 parts, float z, float radius)
{
	TArray<FVector> res;
	...
	{
		float x, y;
		float degreeInRadian = (360.0 / (float)parts) * (float)part * DEGREE_TO_RADIAN;
...
USkeletalMesh* USkeletalMeshLoader::GenerateBlob(USkeleton* BaseSkeleton, TArray<UMaterialInterface*> mats) {
	...
	int32 breakOff = 20;
	float growth = circleRadius / breakOff;
	...
	for (int32 v = 0; v < nVertices; v++)
	{
		...
		float uvx = ((mSurface.Vertices[v].Y + circleRadius) * 0.5)/ circleRadius;
		float uvy = (mSurface.Vertices[v].Z)/zMax;
